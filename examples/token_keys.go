...
import (
	...
	"github.com/murray-distributed-technologies/go-hashchain/keys"
...
func main() {
	parentWIF, _ := wif.DecodeWIF("")
	destWIF, _ := wif.DecodeWIF("")
	...
	privKey := parentWIF.PrivKey

	hashchain, _ = token.GenerateTokenChain(hashchain)

	for i := 0; i < tokenNum; i++ {
		hashchain.LastToken = hashchain.CurrentToken
		hashchain, _ = token.GetNextToken(hashchain)
	}
	if tokenNum > 1 {
		privKey, _ = keys.NewPrivateKey(hashchain.LastToken, privKey)

	}
	destPrivKey, _ := keys.NewPrivateKey(hashchain.CurrentToken, destWIF.PrivKey)

	changeAddress, _ := bscript.NewAddressFromPublicKey(privKey.PubKey(), true)
	address, _ := bscript.NewAddressFromPublicKey(destPrivKey.PubKey(), true)
	...
	rawTxString, err := transaction.CreateTokenKeysTransaction(utxo, privKey, address.AddressString, changeAddress.AddressString, amount)
	if err != nil {
		fmt.Println(err)
	}
